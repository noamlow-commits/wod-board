...
const FIXTURES = [
  ...
  { name: "xsets_written_total_beats_xN", note: "2026-08-20 (Noam, from the gym TV). The SISTER of cardio_written_total_beats_xN, on the other interval path. 'every M:SS x N sets' is claimed by mmssXmRe, which runs BEFORE the every-matchers and consume()s the span — so everyExpRe, the path that has always honoured a written total, never sees the line. mmssXmRe had only two precedence tiers ('(N Rounds)' then the header ×N) and no writtenTotalMin() at all, so a coach writing '(30 min total)' got a clock of ONE THIRD her block and nothing anywhere said so. The hashfirst_stations_rounds note above already asserted this equivalence — '(N Rounds) … (like (N min total) already did)' — while it was false on this exact path; that claim is now true. Numbers chosen so the written total is the ONLY source that can produce them: 20 min ÷ 2:00 = 10 intervals, which is neither the literal ×3 nor the 3 sets × 2 stations = 6 the rotation math would give. Both wrong readings are locked out by forbidTimers. ⚠️ This fixture deliberately says NOTHING about what 'x N sets' means when NO total is written — that is the coach's call (TIMER_ROADMAP §1), and evey_typo_explicit_rounds still locks the literal reading byte-for-byte.",
    expectTimers: ["2:00 ×10"],
    forbidTimers: ["2:00 ×3", "2:00 ×6", "Every 2:00 ×3 (6′)"],
    rows: [["", "WOD"],
           ["", "every 2:00 x 3 sets (20 min total)\n1# 12 cal row\n2# 10 push up"]] },
  ...
  { name: "warmup_header_carries_its_duration", note: "coach's real board (2026-08-13, from the gym TV, Noam): part 1 opens with '8 min WARM UP :' and produced NO clock — she wrote the length of the block on the block's own header line and the board ignored it. TWO independent guards had to be crossed, which is why it survived: (1) `isLabelOnly` skipped any line ending in ':' so the scan could reach a duration written BENEATH a label ('warmup:' → '6 min leg mobility'). The trailing ':' is a proxy for 'this line has no content of its own', and the two shapes — 'is a label' and 'carries a duration' — were tested independently, so a line that is BOTH was skipped as if it were empty. `!hasTiming` now settles it: a line carrying timing is never label-only. (2) even with the ':' removed, 'warm up' sat in the leading-duration rule's pace-descriptor blacklist alongside easy/recovery/cool down. A block NAME is not a pace descriptor — 'easy' modifies HOW the work is done, 'WARM UP' says WHICH block this is — so warm up/cool down are out and easy/recovery stay. ── THE REAL LESSON IS WHY NO FIXTURE COULD HAVE CAUGHT THIS. The same blacklist existed a SECOND time, drifted (`warm[\\s-]?up` vs `warm\\s*up|warmup`), inside extractTimingFacts — the shadow channel whose whole purpose is the unexplained-facts assertion, i.e. proving no written duration was dropped. It recorded '8 min WARM UP' as `ignored: \"pace\"`. So a fixture of this exact cell, written any time in the last months, would have PASSED with an empty timer list: the measurement channel carried the very exclusion it was meant to audit, and could not contradict it. Both sites now build from the shared PACE_DESCRIPTOR const, so the audit and the rule cannot disagree again. ── NEGATIVE CONTROL: `set_wave_lift` ('20 min easy row' → still no timer, via `easy`) and `minutes_on_off_intervals` / `leading_duration_behind_labels` (their 'warm up tabata' and 'warmup:' lines carry no duration, so they are still skipped as labels and the 6′ clock beneath still lands). Both pass byte-for-byte. The '20 sec shoulder stretch' deliberately gets no clock of its own — it is one movement inside the eight minutes, not a block.",
    expectTimers: ["8′ WARM UP"],
    forbidTimers: ["8′ WARM UP :", "20″ shoulder stretch"],
    ignoreFacts: ["20 sec"],
    rows: [["", "1"],
           ["", "8 min WARM UP :\n200 m run\n20 S.U/DU\nOHS DB- 5 EACH HAND\n10 סיבובי מקל\n20 sec shoulder stretch"]] },
  { name: "emom_bare_e_shorthand", note: "coach's real board (2026-08-13, from the gym TV, Noam): column B of the snatch part reads 'e 1:30 x 7' over a 3-movement complex @70% and produced NO clock — a 10:30 EMOM lost to one absent token. She abbreviates 'every' to a bare 'e', and the existing shorthand regex required the literal 'mom' after the time ('e 1:30 mom x 8'); `mmssXmRe` could not take it either, since that path demands a trailing 'sets'/'rounds' to keep '5:00 x 3 burpees' out. Same silent-total-loss shape as the EVEY typo: one unrecognised character costs the whole timer, and the DISPLAY layer meanwhile painted '1:30' with its red time badge, so the board looked like it had understood (structural cause #7). The 'mom' is now optional. ⚠️ The bare 'e' deliberately does NOT join EVERY_WORD even though it IS her 'every': EVERY_WORD has a consumer outside detection — the layout layer's isFormatHeader splices it into `\\b(amrap|emom|…)\\b`, where an 'e' alternative matches any stray letter and would turn it into a block boundary, tearing a superset across columns. A bare 'e' is only 'every' when a clock time AND a round count follow it; that anchor is part of the token, which is precisely what the position-free EVERY_WORD cannot express. Unlike the warm-up fixture above, this one WOULD have been caught by the unexplained-facts assertion the day it landed — 1:30 is an unignored duration reaching no clock — it simply never had a fixture. Column A is the coach's verbatim progression text and must stay clockless: its '(2+3+4+5+7)' and '(Start-7)' are set-position notes, not durations or rounds.",
    expectTimers: ["EMOM 1:30 ×7"],
    forbidTimers: ["Every 1:30 ×7 (11′)", "1:30 ×7"],
    rows: [["", "A", "B"],
  ...
  { name: "interval_blocks_chain_keeps_intervals",
    note: "coach's real sheet (2026-08-31, VERBATIM, WOD column '2' — 'Endurance Day'): two 'every 3:00 x5 sets' blocks around a written '3:00 rest'. Noam: it is ONE continuous 33′ piece — 5×3:00, a 3:00 rest, 5×3:00 — not two clocks. TWO separate defects in one cell. (1) The chain never fired at all: lineSplitRe's concat repair cuts 'every 3:00 x5 sets' into 'every 3:00 x' | '5 sets' (the 'x' is a letter, '5 sets' is digits-space-letters), and buildWorkoutTimeline is the one LINE-scoped consumer of that spec — so the ×N never reached its own matcher, the timeline held zero work phases, and the board showed two 15′ clocks with the written 3:00 rest on nobody's. Every whole-cell TEXT scan is immune by construction (mmssXmRe's `\\s*` spans the break), which is exactly why it survived: the same family as cardio_written_total_beats_xN. Verified counterfactually, not read — the identical lines UNSPLIT chained immediately. (2) The chain that then fired was still WRONG: the uniform builder flattens each 'every 3:00 ×5' into one 15′ slab, so ten written interval starts vanish — a coach whose block says 'every 3:00' gets no cue on any third minute, worse than the two clocks it replaced. So a work phase now carries the interval the coach WROTE (intervalSeconds) and the chain expands it into that many phases: 11 × 3:00 with the rest sixth, rounds 1..10. Nothing invented — her own numbers regrouped; a work phase with no written interval falls back to the flat chain rather than have a beat guessed for it. forbidTimers locks out BOTH wrong answers: the pre-fix pair of '3:00 ×5' clocks and the flat 'Every ×2 · 15′ work / 3′ rest'. ⚠️ RUNTIME half, invisible to this harness (see test/timer-nav.mjs): the tick's phase-transition sound keyed on the phase TYPE, so back-to-back WORK phases were silent — detection can be perfect here and the clock still say nothing on nine of its ten starts.",
    expectTimers: ["Every 3:00 ×5 · 3′ rest · ×5 (33′)"],
    forbidTimers: ["3:00 ×5", "Every ×2 · 15′ work / 3′ rest"],
    rows: [["", "2"],
           ["", "Endurance Day\nevery 3:00 x5 sets\n300 run\n10 pull up/ring row\n12/10 cal\n\n3:00 rest\n\nevery 3:00x5 sets\n20 wall ball\n10 db snatch\n"]] },
  // ── The minute map (2026-09-09) ──────────────────────────────────────────
  { name: "minute_map_window",
    note: "coach's real WOD cell, VERBATIM from getWorkoutSheet (2026-09-09, column '3'). She declares a repeating five-minute WINDOW ×3 and then assigns the minutes INSIDE it: 1-3 an AMRAP of two movements, 4 a single movement, 5 rest. The board read the two halves independently and neither described the workout — `AMRAP 3′` (the opening line, and index 0, i.e. the ⏱↻ DEFAULT) beside `5′ ×3 (15′ total)`, an EMOM that beeps once every FIVE minutes. So the two transitions the class actually needs called, 3:00 and 4:00 of every window, were silent in BOTH clocks, and whichever one she started was wrong: the first ends at 3:00 of 15, the second never says a word until the set is over. Now ONE `tabata` with an explicit 9-phase schedule (3′ work · 1′ work · 1′ rest) ×3 = 15:00, so every written minute boundary gets its cue and ⏭ הבא can skip a segment. Nothing is inferred: 'min 4:' STATES that the fourth minute begins (a boundary at 3:00), 'min 5:' one at 4:00, and the window closes the last — the opening's written '3 min amrap' only has to AGREE with the first marker, and a cell where it doesn't is left alone. forbidTimers locks out both halves of the old reading. ⚠️ The trailing rest is deliberately NOT popped the way a chain pops its own: it is a written minute of a written 5:00 window, and dropping it would contradict her 'every 5 min x3' = 15:00.",
    expectTimers: ["5′ ×3 · 3′/1′/1′ (15′)"],
    forbidTimers: ["AMRAP 3′", "5′ ×3 (15′ total)", "5′ ×3 · 3′/1′/1′ (14′)"],
    rows: [["", "3"],
           ["WOD", "every 5 min x3 sets\n3 min amrap:\n12 wall ball\n12 alt db snatch\nmin 4:max burpee over db\nmin 5: rest"]] },
  { name: "minute_map_written_range",
    note: "the same shape with the opening written as a RANGE ('min 1-3: amrap') instead of as a leading duration. The range END is a cross-check, not the source of the boundary: minutes 1-3 must end exactly where 'min 4:' says the next segment starts, and a range that disagrees returns null rather than picking a side. Also covers the marker at minute 1 (no separate opening segment) and the '-' separator form.",
    expectTimers: ["5′ ×3 · 3′/1′/1′ (15′)"],
    forbidTimers: ["5′ ×3 (15′ total)", "AMRAP 3′"],
    rows: [["", "3"],
           ["WOD", "every 5:00 x3 sets\nmin 1-3: amrap\n12 wall ball\n12 alt db snatch\nmin 4 - max burpee over db\nmin 5: rest"]] },
  { name: "minute_map_contradiction_left_alone",
    note: "THE NEGATIVE CONTROL, and the one that matters: she writes a FOUR-minute opening ('4 min amrap:') but then marks 'min 4:' — which says the fourth minute is a segment of its own, i.e. the opening is three. Those two statements cannot both be true, and resolving one in favour of the other is exactly the invented value the whole no-invented-timer-values rule exists to forbid. The minute map declines and the cell falls back to whatever the pre-existing detectors make of it — a worse clock, but an honest one, and the contradiction stays visible in her own text where she can fix it. If a future widening ever 'helpfully' picks a side, this fixture is the one that fails.",
    forbidTimers: ["5′ ×3 · 3′/1′/1′ (15′)", "5′ ×3 · 4′/1′ (15′)"],
    expectTimers: ["AMRAP 4′", "5′ ×3 (15′ total)"],
    rows: [["", "3"],
           ["WOD", "every 5 min x3 sets\n4 min amrap:\n12 wall ball\nmin 4:max burpee over db\nmin 5: rest"]] },
  { name: "minute_map_needs_a_separator",
    note: "the other negative control, from the SAME board: her column '2' heads a back-squat block with a bare 'min 12', meaning TWELVE MINUTES — not minute 12 of anything. The marker pattern therefore requires a separator after the number (':' '.' '-' ')'), which is what keeps a unit-first block duration out of the minute map. Without the guard this cell would have claimed a window and produced a schedule out of a number that is not a minute index at all. The ×3 interval clock is unchanged, byte-for-byte with what shipped before the minute map existed.",
    expectTimers: ["5′ ×3 (15′ total)"],
    forbidTimers: ["5′ ×3 · 3′/1′/1′ (15′)"],
    rows: [["", "3"],
           ["WOD", "every 5 min x3 sets\nmin 12\n12 wall ball\n12 alt db snatch"]] },
  // ── A line that is nothing but a duration (2026-09-09) ───────────────────
  { name: "bare_block_duration_then_rest",
    note: "coach's real WOD cell, VERBATIM (2026-09-09, column '2'), the one beside `minute_map_window`: ' min 12' / 'BACK SQUAT: 1 and half' / '4 reps @ 75-80%' / … / 'rest 2:00 min'. Twelve minutes for the squat block and then two minutes rest — and the board put NO clock on ANY of it. Every other duration rule wants the number next to something: an activity ('8 min leg and lat activation'), a format keyword, a multiplier ('WARM UP x 6 min'), the word `work`, a `t.c`. A line that is ONLY a duration matched none of them, and it is the one shape with no second meaning available to it — which is exactly why it is safe where roadmap Q6's 'WARM UP 6 min' is not. Both word orders count, because the order is hers. Noam, 2026-09-09: 'את ה-12 מעל הבק סקווט בחלק השני צריך לעשות כ-for time' + 'ואז 2 דקות מנוחה', and he chose ONE button with an automatic transition over a literal count-up plus a second clock. So the rest written beneath the block belongs to it: 12:00 → beep → 2:00 rest, 14:00 total. ⚠️ The price is the count-up: `fortime` is the only count-up type and a sequence cannot mix directions (TIMER_ROADMAP §5 — a queue engine, not a patch), so the compound form counts the 12 DOWN. That trade is his, made knowingly; see `bare_block_duration_alone` for the shape that does not pay it.",
    expectTimers: ["TC 12′ · 2′ rest (14′)"],
    forbidTimers: ["TC 12′ · For Time", "12′", "2′ rest"],
    rows: [["", "2"],
           ["WOD", " min 12\nBACK SQUAT: 1 and half\n4 reps @ 75-80%\nלשמור על איכות- לא להגיע לכשל\n(גרג)\n\nrest 2:00 min\n"]] },
  { name: "bare_block_duration_alone",
    note: "the same block with NO rest written under it. There is no sequence, so nothing has to be traded for the automatic transition and the block stays a TRUE For Time — count-up, stopping at the 12:00 cap, which is what the coach means by 'for time'. Keeping this pair side by side is the point: the countdown in the sibling fixture is the ENGINE's constraint (one direction per sequence), not a decision about what a block duration means. If a queue engine ever lands, this is the fixture that says what the compound form should have been.",
    expectTimers: ["TC 12′ · For Time"],
    forbidTimers: ["TC 12′ · 2′ rest (14′)", "AMRAP 12′", "12′"],
    rows: [["", "2"],
           ["WOD", "min 12\nBACK SQUAT: 1 and half\n4 reps @ 75-80%"]] },
  { name: "bare_block_duration_is_last_resort",
    note: "NEGATIVE CONTROL 1 — the `results.length === 0` guard. The same bare 'min 12' above a block that already declares its own format: another detector owns this cell, and a count-up beside its AMRAP would be a ghost clock of exactly the kind `station_amrap_nested_in_rotation` and the `alreadyHaveThisLong` guard exist to prevent. The bare-block rule is a last resort BY CONSTRUCTION, and this is what says so.",
    expectTimers: ["2:00 ×5"],
    forbidTimers: ["TC 12′ · For Time", "TC 12′ · 2′ rest (14′)"],
    rows: [["", "2"],
           ["WOD", "min 12\nBACK SQUAT: 1 and half\nevery 2:00 x5 sets\n4 reps @ 75-80%"]] },
  { name: "bare_block_duration_must_lead",
    note: "NEGATIVE CONTROL 2 — the position guard, and the likeliest misfire if anyone ever relaxes it. A bare duration HEADING a block names that block; the same token sitting further down a list is a note about one movement, or a leftover, and claiming it would put a clock on cells all over her sheet. The rule therefore reads the FIRST content line only (pure label lines skipped, via the same `isLabelOnly` the leading-duration rule uses — hoisted 2026-09-09 rather than copied, so 'what counts as a label' cannot drift between the two). Here the squat line comes first, so the '12 min' below it is left alone and the cell is correctly clockless.",
    expectTimers: [],
    forbidTimers: ["TC 12′ · For Time", "12′"],
    ignoreFacts: ["12 min"],
    rows: [["", "2"],
