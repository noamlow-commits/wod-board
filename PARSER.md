...
**Part-level cross-cell time cap + no-invented `×N` for a bare "every X:XX" (added 2026-07-27, fixture `fortime_pacing_cap_columns`).** The coach writes the `t.c N` of a workout unit at its **END** — sometimes in the same column as the work, sometimes in a **separate column** of the same part. Her "for time: | 3000 m run / every 4:00 / 5 burpee / 8 push up / 20 d.u | rx+ 4000 m run / t.c 35" sheet parsed as `Every 4:00 ×4 (16′)` — the ×4 was **invented** by the exercise-line fallback reading the for-time list (including the 3000 m run) as 4 stations — while the `TC 35′` sat disconnected in the other column. Three rules:
- **Canonical cap matcher** — the two cap regexes now live ONCE in `capSecondsFromLine(line)`; `detectTimers` (content lines), `extractTimerConfigs` (cell header) and `scanPartCapSeconds` (whole part) all call it, so the 3-parallel-places rule is enforced by construction. `scanPartCapSeconds(row.cells)` scans every cell of the row (lines AND headers) and the result is threaded as `extractTimerConfigs(lines, header, partCapSeconds)` → `detectTimers(lines, partCapSeconds)` by BOTH callers: `renderWorkout` and the harness (goldens match production).
