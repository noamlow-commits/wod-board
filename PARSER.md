...
**`d.u` is DOUBLE UNDERS, not group "D" (fixed 2026-09-10, fixture `du_is_not_group_d`).** The group rule is "single letter + separator", which read the coach's `d.u` line as group **D** whose content is `u`: it badged the D — splitting her initials on the TV — *and* opened a group boundary. One shared literal, `DU_ABBR_RE`, now guards **both** owners of that pattern (`parseLine` for the badge, `isGrpLine` for the division), exactly like the `t.c` guard beside each of them; it covers `d.u` / `d.u.` / `du` / `D-U` / `d.u's`. The `\b` after the `u` is the line: `D. Upper body` still reads as a real group D. ⚠️ Fixing only the badge would have left the phantom column boundary in place — the badge and the layout are two consumers of one rule, and the fixture asserts both.

**The ≥3-lines-per-side floor is now actually implemented (2026-09-10).** The rule above ("never one line beside nine") was documented but the 2026-07-02 splitter rewrite shipped a floor of **1**, so a lopsided break was legal. The floor is now `segReal <= MAX_PER_COL ? 3 : 1` — it binds while the split is *optional* (the real content still fits one column, so a lopsided break buys nothing) and relaxes once the content genuinely overflows and we must break somewhere. Measured in **real lines** on purpose: `colSize` counts the section header and blank-line spacers too, and on the coach's CARDIO cell those two items alone (9 real lines, cap 9, `colSize` 11) pushed it over the cap — a spacer spawning the very ghost column the balance rule exists to prevent. With the floor, a 2-line head beside an atomic 7-line group stays **one column**.
